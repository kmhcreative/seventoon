...
/*	SevenToon Widgets
*/
class seventoon_chapters_widget extends WP_Widget {
	...
	public function __construct() {
		$widget_ops = array(
			...
			'description'                 => __( 'A list or dropdown of posts-as-comics chapter categories (not custom taxonomy).', 'seventoon' ),
			'customize_selective_refresh' => true,
			'show_instance_in_rest'       => true,
		);
		parent::__construct( 'chapters', __( 'SevenToon Chapters', 'seventoon' ), $widget_ops );
	}
	...
	public function widget( $args, $instance ) {
		...
		$default_title = __( 'Chapters', 'seventoon' );
		...
		if ( $dropdown ) {
			...
			$cat_args['show_option_none'] = __( 'Select Category', 'seventoon' );
...
/* ]]> */
			...
			if ($thumbnails == 'first' || $thumbnails == 'last'){
				$my_walker = new CategoryThumbnail_Walker();
	...
	public function form( $instance ) {
		...
		?>
		<p>
			<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:', 'seventoon' ); ?></label>
			<input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo esc_attr( $instance['title'] ); ?>" />
		</p>
		<p>
			<label for="<?php echo $this->get_field_id( 'include' ); ?>"><?php _e( 'Include: (optional comma-separated category IDs)', 'seventoon' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'exclude' ); ?>"><?php _e( 'Exclude: (optional comma-separated category IDs)', 'seventoon' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'order' ); ?>"><?php _e( 'Order: (ASC | DESC )', 'seventoon' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'orderby' ); ?>"><?php _e( 'Order By:', 'seventoon' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'thumbnails' ); ?>"><?php _e( 'Thumbnails: (first | last | none)', 'seventoon' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'type' ); ?>"><?php _e( 'Post Type:', 'seventoon' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'taxonomy' ); ?>"><?php _e( 'Taxonomy: ', 'seventoon' ); ?></label>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id( 'dropdown' ); ?>"><?php _e( 'Display as dropdown', 'seventoon' ); ?></label>
			<br />

			<input type="checkbox" class="checkbox" id="<?php echo $this->get_field_id( 'count' ); ?>" name="<?php echo $this->get_field_name( 'count' ); ?>"<?php checked( $count ); ?> />
			<label for="<?php echo $this->get_field_id( 'count' ); ?>"><?php _e( 'Show post counts', 'seventoon' ); ?></label>
			<br />

			<input type="checkbox" class="checkbox" id="<?php echo $this->get_field_id( 'hierarchical' ); ?>" name="<?php echo $this->get_field_name( 'hierarchical' ); ?>"<?php checked( $hierarchical ); ?> />
			<label for="<?php echo $this->get_field_id( 'hierarchical' ); ?>"><?php _e( 'Show hierarchy', 'seventoon' ); ?></label>
			<br />
			...
			<label for="<?php echo $this->get_field_id( 'emptychaps' ); ?>"><?php _e( 'Hide Empty Chapters', 'seventoon' ); ?></label>
...
function seventoon_chapter_thumbnail($chapter,$firstlast = 'first',$taxonomy = 'category', $post_type = 'post'){
	if ( $firstlast == 'first' ){
		$order = 'ASC';
...
class CategoryThumbnail_Walker extends Walker_Category {
	...
	function start_el(&$output, $item, $depth=0, $args=array(), $current_object_id = 0){
        $image = seventoon_chapter_thumbnail($item,$args['thumbnails'],$args['taxonomy'],$args['type']);
			if (!empty($image)){
...
class seventoon_promo_slider extends WP_Widget {
 	/**
	 * Sets up a new Categories widget instance.
	 *
	 * @since 2.8.0
	...
	public function __construct() {
		$widget_ops = array(
			...
			'description'                 => __( 'A promotional image slider.', 'seventoon' ),
			'customize_selective_refresh' => true,
			'show_instance_in_rest'       => true,
		);
		parent::__construct( 'promos', __( 'SevenToon Promos', 'seventoon' ), $widget_ops );
	}
    ...
    public function form( $instance ) {
        ...
        ?>
        <p class="slider_title"><?php _e('Title', 'seventoon');?>: <input type="text" style="width:100%;" name="<?php echo esc_attr( $this->get_field_name( 'slider_title'));?>" value="<?php echo $slider_title; ?>"></p>
        <a class="seventoon_upload_image_media" href="javascript:void(0)"><?php _e("Add Images","seventoon");?></a><br/>
        <table class="seventoon_multi_image_slider_table_wrapper"> 
            ...
            ?>
             <tr class="seventoon_no_images" <?php if( isset($image_url) && count($image_url)>=1){?> style="display:none;" <?php }?> >
                            <td colspan="3">
                                <?php _e("No images selected", 'seventoon');?>
                            </td>
            </tr>
            </tbody>
        </table>
                 ...
                 if ($count == 0){
                    ...
                    if (!empty($image_src)) { ?>
                       ...
                       <?php  if(!empty($linkval[$count])) {?>
                            <a href="<?php if(!empty($linkval[$count])){echo esc_url($linkval[$count]);}?>" <?php if($targetval[$count] == 'newtab'){echo "target='_blank'";}?> title="<?php _e("Click here", 'seventoon');?>" >
...
// Now register these bad boys...
function seventoon_register_widgets(){
	register_widget('seventoon_chapters_widget');
	register_widget('seventoon_promo_slider');
}
add_action( 'widgets_init', 'seventoon_register_widgets' );